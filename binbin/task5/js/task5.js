...
function removeNum(position){
    var container = document.getElementById('container');
    var firstDiv = container.children[0]; //读取最左边的元素
    var lastDiv = container.lastChild;  //读取最右边的元素

    if(container.children.length==0){
        alert("队列没有元素");
    }
    else if(position=='left'){
        var firstHeight= firstDiv.style.height.replace('px',''); //获取值
        alert('删除最左边的元素:'+firstHeight);
        container.removeChild(firstDiv);
    }
    else if(position=='right'){
        var lastHeight = lastDiv.style.height.replace('px','');
        alert('删除最右边的元素:'+lastHeight);
...
function sort(){
    ...
    timer = setInterval(run,10); //执行异步操作
